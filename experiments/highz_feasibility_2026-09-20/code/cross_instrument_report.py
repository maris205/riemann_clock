#!/usr/bin/env python3
"""Produce the bounded cross-instrument report after final saved endpoints exist."""
from pathlib import Path
import json,hashlib
from cross_instrument_summarize import summarize
ROOT=Path(__file__).resolve().parents[1];D=ROOT/'results/cross_instrument'
REQUIRED=['uves_refined_null_continued','uves_expected_null_continued','uves_expected_alternative_continued','uves_opacity_null','uves_opacity_alternative']
for name in REQUIRED:assert (D/(name+'.json')).exists(),name
s=summarize();look={f['name']:f for f in s['fits']}
rows=[];statusrows=[]
for kind,title in [('refined','统计误差'),('expected','档案建议的 expected-fluctuation 误差'),('opacity','同上，并允许强线不透明度各变动 ±10%')]:
 c=s[kind];n=look[c['null']];a=look[c['alternative']]
 rows.append(f"| {title} | {n['chi2']:.3f} | {a['chi2']:.3f} | {c['delta_chi2']:.3f} | {a['shift2382_m_s']:.1f} | {a['shift2600_m_s']:.1f} |")
 for row in [n,a]:
  statusrows.append(f"| `{row['name']}` | {'达到终止容差' if row['success'] else '达到迭代上限'} | {row['nfev']} | {row['optimality']:.4g} | {row['rank']}/{row['npar']} | {row['bound_count']} |")
v=json.loads((D/'validation.json').read_text())
stat=look[s['refined']['alternative']];statn=look[s['refined']['null']]
notes=f'''# UVES 独立仪器复核：条件性同方向线索，尚未排除常规解释
...
|---|---|---:|---:|---:|---:|
{chr(10).join(statusrows)}

初始统计误差 H0 分支停在 χ² = {look['uves_refined_null']['chi2']:.3f}；从 H1 的气体参数重新启动 H0 后得到更低的解。这个变化本身说明不能只挑一个初始化状态就宣称显著性。相对位移的优化解与仪器、气体参数的局部解存在耦合。

## 关键的常规解释与限制

档案的名义分辨率 R≈53696、弧灯分辨率 R≈70723，都不是独立测得的类星体有效线形。直接固定名义高斯线宽时 H0 χ²≈1399；允许拟合仪器线宽后目标函数降低数百，证明仪器线形假设会显著影响此项检验。

最终统计误差 H1 的高斯 FWHM 分别为 {stat['lsf_fwhm_km_s'][2374]:.4f}、{stat['lsf_fwhm_km_s'][2382]:.4f}、{stat['lsf_fwhm_km_s'][2600]:.4f} km/s。2374 的宽度比例达到预设下限 0.65（相对于名义宽度），对应有效 R≈82609。不能把这个边界上的拟合值当作仪器标定测量。气体速度和宽度也有活动边界，因此局部协方差误差不能涵盖全部不确定性。
...
独立复核脚本为 `code/cross_instrument_validate.py`。在本报告生成时，机器报告为 {v['status']}，{v['passed']}/{v['count']} 项检查通过；最终审计覆盖的具体端点和 SHA-256 均在 `results/cross_instrument/validation.json`，检查通过只表示实现/保存数组可复算，不表示物理推断已获验证。
...
'''
(ROOT/'reports/cross_instrument_results_cn.md').write_text(notes)
prov=dict(adapter_sha256=hashlib.sha256((ROOT/'code/cross_instrument_uves.py').read_bytes()).hexdigest(),
          base_model_sha256=hashlib.sha256((ROOT/'code/espresso_conventional_null.py').read_bytes()).hexdigest(),
          summary_sha256=hashlib.sha256((D/'summary.json').read_bytes()).hexdigest(),
          validation_sha256=hashlib.sha256((D/'validation.json').read_bytes()).hexdigest(),
          note='Hashes identify the final reproducible implementation and saved replay audit, not an assertion that all optimization endpoints converged.')
(D/'delivery_provenance.json').write_text(json.dumps(prov,indent=2)+'\n')
print(ROOT/'reports/cross_instrument_results_cn.md')
